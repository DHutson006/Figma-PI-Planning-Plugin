...
### Template Types
- 🎯 **Theme**: High-level business objectives with business value
- 🚀 **Initiative**: Strategic initiatives that span multiple epics
- 📅 **Milestone**: Track key project milestones with target dates
- 🏗️ **Epic**: Large work items that span multiple sprints
- 📋 **User Story**: Standard user story format (As a/I want/So that)
- ✅ **Task**: Task items with story points and acceptance criteria
- 🔬 **Spike**: Research and investigation items
- 🧪 **Test**: Test cases with Given/When/Then format
...
- **Jira Base URL**: Enter the base URL of your Jira instance (e.g., `https://myjira.disney.com/`). This is used to create hyperlinks for Jira issues.
...
5. Use the template cards to insert new planned tickets into your board. As the facilitator, you can add the template tickets to the board and any users invited to the board can contribute to planning by copying a template ticket or an imported ticket. 
...
```
.
├── manifest.json          # Plugin configuration (FigJam only)
├── package.json           # Dependencies and build scripts
├── tsconfig.json          # TypeScript compiler configuration
├── ui.html                # Plugin UI (tool pane interface)
├── code.js                # Bundled output (generated by build)
├── Deploy Docs/           # Deployment documentation
└── src/
    ├── code.tsx           # Main plugin entry point and message handler
    ├── card-creation.ts   # Template card creation logic
    ├── config.ts          # Configuration constants (sizes, colors, timing)
    ├── templates.ts       # Template definitions
    ├── types.ts           # TypeScript type definitions
    ├── utils.ts           # Utility functions (validation, helpers)
    ├── errors.ts          # Structured error handling with error codes
    ├── logger.ts          # Centralized logging service
    ├── validation.ts      # Enhanced input validation and sanitization
